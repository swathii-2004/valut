...
// Partner lookup now scoped to vault_members instead of "all other users".
'use strict';
...
const { verifyVaultMember } = require('../middleware/vault');
...
// GET /api/profile/partner — vault-scoped partner lookup
router.get('/partner', authMiddleware, verifyVaultMember, async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT u.id, u.email, u.display_name, u.avatar_filename
             FROM vault_members vm
             JOIN users u ON u.id = vm.user_id
             WHERE vm.vault_id = $1 AND vm.user_id != $2`,
            [req.vaultId, req.user.sub]
        );
        if (!result.rows.length) return res.status(404).json({ error: 'No partner yet' });
        const user = result.rows[0];
        return res.json({
            id          : user.id,
            email       : user.email,
...
// GET /api/profile/avatar/:userId — serve avatar (no auth needed for display)
...
router.put('/avatar', authMiddleware, upload.single('avatar'), async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE users SET avatar_filename = $1 WHERE id = $2`, [req.file.filename, req.user.sub]);
        return res.json({ avatar_url: `/api/profile/avatar/${req.user.sub}`, avatar_filename: req.file.filename });
    } catch (err) {
...
router.put('/display-name', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE users SET display_name = $1 WHERE id = $2`, [display_name.trim(), req.user.sub]);
...
router.post('/push-token', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE users SET push_token = $1 WHERE id = $2`, [token, req.user.sub]);
