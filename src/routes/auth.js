...
const REFRESH_TOKEN_EXP = 30 * 24 * 60 * 60 * 1000;
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user.locked_until && new Date(user.locked_until) > new Date()) {
            await writeAuditLog({ userId: user.id, action: 'login_failure', ip: req.ip, userAgent: req.headers['user-agent'], success: false });
        ...
        if (!isValid) {
            ...
            const lockout  = newCount >= MAX_ATTEMPTS ? new Date(Date.now() + LOCKOUT_MINUTES * 60000) : null;
            await pool.query('UPDATE users SET failed_attempts = $1, locked_until = $2 WHERE id = $3', [newCount, lockout, user.id]);
            await writeAuditLog({ userId: user.id, action: 'login_failure', ip: req.ip, userAgent: req.headers['user-agent'], success: false });
            return res.status(401).json({ error: 'Invalid credentials' });
        }

        await pool.query('UPDATE users SET failed_attempts = 0, locked_until = NULL, last_login = NOW() WHERE id = $1', [user.id]);
        ...
        await writeAuditLog({ userId: user.id, action: 'login_success', ip: req.ip, userAgent: req.headers['user-agent'], success: true });
...
router.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        await writeAuditLog({ userId: user.id, action: 'token_refresh', ip: req.ip, userAgent: req.headers['user-agent'], success: true });
...
router.post('/logout', authMiddleware, async (req, res) => {
    ...
    try {
        if (refresh_token) {
            const tokenHash = crypto.createHash('sha256').update(refresh_token).digest('hex');
            await pool.query('UPDATE refresh_tokens SET is_revoked = TRUE, revoked_at = NOW() WHERE token_hash = $1', [tokenHash]);
        }
        if (req.user) {
            await writeAuditLog({ userId: req.user.sub, action: 'logout', ip: req.ip, userAgent: req.headers['user-agent'], success: true });
        }
